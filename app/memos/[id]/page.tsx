	...
	useEffect(() => {
		const fetchMemo = async () => {
			try {
				...
				const memoCategory = categories.find(
					(c: any) => c.id === memoData.category,
				);
				const memoTags = tagsData.filter((t: any) =>
					memoData.tags.includes(t.id),
				);
	...
	return (
		...
		>
			...
			<div className="bg-gray-900 rounded-lg p-6 border border-gray-800">
				<h1 className="text-2xl font-bold text-gray-100 mb-4">{memo.title}</h1>
